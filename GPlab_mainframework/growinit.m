function [pop,lastid]=growinit(n,lastid,maxlevel,oplist,oparity,depthnodes,dimension)
%GROWINIT    Creates a new GPLAB population with the grow method.
%   GROWINIT(POPSIZE,LASTID,MAXLEVEL,OPERATORS,ARITY,DEPTHNODES)
%   returns a population of POPSIZE new individuals for the GPLAB
%   algorithm, with unique identifiers beginning on LASTID+1 and
%   randomly created tree representations built with the grow
...
%
%   [POPULATION,LASTID] = GROWINIT(POPSIZE,LASTID,MAXLEVEL,
...
%      MIT Press (1992).
%
%   See also FULLINIT, RAMPEDINIT, INITPOP, NEWIND
%
...
for i=1:n
   ntry=1;
   while ntry<=20
   	[pop(i),tmplastid]=newind(lastid,maxlevel,oplist,oparity,0,depthnodes,dimension);
      % (0 = grow method - the new tree level does not have to be exactly maxlevel)
